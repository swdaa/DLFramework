...
bool operator==(const Parameter& lhs, const Parameter& rhs)
{
    ...
    if (lhs.type == 10 && lhs.c == rhs.c) return true;

    if (lhs.type == 11 && lhs.ac == rhs.ac) return true;
...
std::string Parameter::encode_to_string(const Parameter& param)
{
    ...
    if (param.type == 5)
    {
        ...
        for (size_t i = 0; i < param.ai.size(); i++)
        {
            ...
            if (i + 1 != param.ai.size()) s += std::string(",");
        }
        s += std::string(")");
        return s;
    }
    if (param.type == 6)
    {
        ...
        for (size_t i = 0; i < param.af.size(); i++)
        {
            ...
            s += std::string(buf);
            if (i + 1 != param.af.size()) s += std::string(",");
        }
        s += std::string(")");
        return s;
    }
    if (param.type == 7)
    {
        ...
        {
            s += param.as[i];
            if (i + 1 != param.as.size()) s += std::string(",");
        }
        s += std::string(")");
        return s;
    }
    ...
    if (param.type == 11)
    {
        ...
        for (size_t i = 0; i < param.ac.size(); i++)
        {
            ...
            s += std::string(buf);
            if (i + 1 != param.ac.size()) s += std::string(",");
        }
        s += std::string(")");
        return s;
    }
...
Operand* Operator::named_input(const std::string& key)
{
    ...
    {
        if (inputnames[i] == key) return inputs[i];
    }
...
static std::string expand_expression(const Operator* op)
{
    ...
    for (int i = (int)tokens.size() - 1; i >= 0; i--)
    {
        ...
        else if (t == "int" || t == "abs" || t == "acos" || t == "acosh" || t == "asin" || t == "asinh" ||
                 t == "atan" || t == "atanh" || t == "ceil" || t == "cos" || t == "cosh" || t == "exp" ||
                 t == "floor" || t == "log" || t == "log10" || t == "neg" || t == "reciprocal" || t == "round" ||
                 t == "rsqrt" || t == "sign" || t == "sin" || t == "sinh" || t == "sqrt" || t == "square" ||
                 t == "tan" || t == "tanh" || t == "trunc" || t == "torch.bool" || t == "torch.float" ||
                 t == "torch.long")
        {
        ...
        else if (t == "atan2" || t == "fmod" || t == "max" || t == "maximum" || t == "min" || t == "minimum" ||
                 t == "pow" || t == "logaddexp")
        {
            std::string binaryop;
        ...
        else if (t == "add" || t == "sub" || t == "mul" || t == "div" || t == "floor_divide" || t == "remainder" ||
                 t == "and" || t == "or" || t == "xor" || t == "lshift" || t == "rshift")
        {
            std::string binaryop;
            if (t == "add") binaryop = "+";
            if (t == "sub") binaryop = "-";
...
static std::string make_slice_expression(const Operator* op)
{
    ...
    if (!pr.empty() && !nr.empty()) return pr + "...," + nr;

    if (pr.empty() && !nr.empty()) return std::string("...,") + nr;
...
int Graph::python(const std::string& pypath, const std::string& pnnxbinpath)
{
        ...
        for (const Operator* op : ops)
        {
            ...
            for (const auto& it : op->params)
            {
                ...
                if (param.type == 5)
                {
                    ...
                    for (size_t i = 0; i < param.ai.size(); i++)
                    {
                        if ((op->type == "nn.AdaptiveAvgPool2d" || op->type == "nn.AdaptiveAvgPool3d" ||
                             op->type == "nn.AdaptiveMaxPool2d" || op->type == "nn.AdaptiveMaxPool3d") &&
    ...
    // load weights
    {
        ...
        for (const Operator* op : ops)
        {
            ...
            for (size_t i = 0; i < attr.shape.size(); i++)
            {
                if (attr.shape[i] == 0) is_empty = true;
            }

            if (is_empty)
            {
                fprintf(pyfp, "        self.%s_%s = torch.from_numpy(np.empty((", sanitize_identifier(op->name).c_str(),
    ...
    // forward body
    {
        for (const Operator* op : ops)
        {
            ...
            if (op->type == "pnnx.SliceIndexes") continue;
            ...
            else
            {
                ...
                for (const auto& it : op->params)
                {
                    ...
                    if (param.type == 5)
                    {
                        ...
                        for (size_t i = 0; i < param.ai.size(); i++)
                        {
                            if ((op->type == "F.adaptive_avg_pool2d" || op->type == "F.adaptive_avg_pool3d" ||
                                 op->type == "F.adaptive_max_pool2d" || op->type == "F.adaptive_max_pool3d") &&
                                it.first == "output_size" && param.ai[i] == 0)
                            {
                                fprintf(pyfp, "None");
                            }
                    ...
                    if (param.type == 11)
                    {
                        ...
                        for (size_t i = 0; i < param.ac.size(); i++)
                        {
                            ...
                            if (i + 1 != param.ac.size() || param.ac.size() == 1) fprintf(pyfp, ",");
                        }
                        fprintf(pyfp, ")");
                    }
                }
    ...
    // export onnx
    {
        ...
        // torch.onnx._export(net, v_0, "test_swin_t.onnx", export_params=True, opset_version=14, input_names=['in0'],
        // output_names=['out0'])
...
void Operand::remove_consumer(const Operator* c)
{
    ...
    if (it != consumers.end()) consumers.erase(it);
}
