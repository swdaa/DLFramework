...
void Tensor<T>::Padding(const std::vector<uint32_t>& pads, T padding_value)
{
    ...
    arma::Cube<T> new_data(this->data_.n_rows + pad_rows1 + pad_rows2, this->data_.n_cols + pad_cols1 + pad_cols2,
                           this->data_.n_slices);
...
void Tensor<T>::Fill(const std::vector<T>& values, bool row_major)
{
    ...
    if (row_major)
    {
        ...
        {
            arma::Mat<T> channel_data_t(const_cast<T*>(values.data()) + i * planes, this->cols(), this->rows(), false,
                                        true);
...
void Tensor<T>::Reshape(const std::vector<uint32_t>& shapes, bool row_major)
{
    ...
    const size_t current_size = std::accumulate(shapes.begin(), shapes.end(), size_t(1), std::multiplies<size_t>());
    CHECK(shapes.size() <= 3);
